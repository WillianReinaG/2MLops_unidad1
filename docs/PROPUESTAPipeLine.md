...
**Repositorio:** [WillianReinaG/2MLops_unidad1](https://github.com/WillianReinaG/2MLops_unidad1) · **Rama:** `unidad3`  
...
La estructura del diagrama sigue el modelo de referencia del curso ([healthPrediction-mlops-U2, rama `entrega3`](https://github.com/rchicangana/healthPrediction-mlops-U2/tree/entrega3)): **Data Pipeline → Develop → Staging → PROD**, con ciclo de re-entrenamiento. Las **herramientas difieren** en CI/CD, almacenamiento de datos, registro de modelos y detección de drift (ver §1.1).

> **MVP ya implementado:** `servicio_estado_clinico/` — no forma parte de esta propuesta ejecutable; es el punto de partida documentado en Semana 1.
...
| **Feature store (offline)** | **Feast** (modo offline) + sklearn `Pipeline` | Contrato único train/serve; un solo artefacto serializado. |
...
| **Enfermedades huérfanas (dominio)** | Patologías **no presentes** en el CSV no pueden inferirse por nombre. | Respuesta `requiere_revision_humana`; cuarentena `data/raw/quarantine/`. |
...
**Suposición D1:** el CSV procesado de cuatro categorías es **representativo suficiente** del dominio académico. Si llegaran cohortes muy distintas, Evidently dispararía re-entrenamiento.
...
**Para qué en este proyecto:** alimentar entrenamiento LightGBM y vincular cada run MLflow a un snapshot `data@vN`.
...
| **Reglas ejemplo** | Presiones &gt; 0; colesterol/glucosa en {1,2,3}; proporción AGUDA dentro de banda histórica |
...
**Trigger:** merge a rama `main` (o `unidad3` en entrega) tras PR aprobado.
...
*Simulación académica — no constituye asesoría clínica.*
